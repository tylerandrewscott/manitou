...
iid_priors = list(pc_prior_prec = list(prec = list(prior = "pc.prec", param = c(5,0.01))),
                  ...
                  pc_prior_cor0 = list(prec = list(prior = "pc.cor0", param = c(0.5,0.01))),
                  pc_prior_cor1 = list(prec = list(prior = "pc.cor1", param = c(0.5,0.75))),
                  loggamma_prior.1.00005 = list(prec = list(prior = "loggamma", param = c(1, 0.00005))),
                  loggamma_prior.1.1 = list(prec = list(prior = "loggamma", param = c(1, 1))),
                  halfnormal_prior = list(prec = list(prior = HN.prior)),
                  halfcauchy_prior = list(prec = list(prior = HC.prior)),
                  halft_prior = list(prec = list(prior = HT.prior)),
...
mod_list = lapply(iid_priors,function(f) {print(f);gc();
  iid_prior_u = iid_prior_y = f
  inla(form0x,family = c('nbinomial', 'betabinomial'),Ntrials = idat$u,
                          control.fixed = list(expand.factor.strategy = "model.matrix"),
                          control.family = famcontrol,
                          #control.inla = cinla,
                          control.results = cres,
