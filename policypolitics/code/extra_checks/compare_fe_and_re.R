...
packages = c('data.table','stringr','tidyverse','sf','lwgeom','ggthemes','tigris','lubridate','MASS','aod','matrixStats','devtools')
...
varnames = c('LCV','demRep','demVote')
  ...
  (extract_comp = ggplot(extract_coefs,aes(x = mean,xmin = `0.025quant`,xmax = `0.975quant`,
   ...
   theme_bw() + 
    ...
    scale_color_tableau(name = 'Specification',palette = 'Color Blind') + 
    guides(shape = 'none',fill = 'none') + 
    ggtitle('Comparing coefficient estimates from random and fixed intercept models') +
    NULL)


ggsave(extract_comp,filename = paste0('policypolitics/tables_figures/figures/figures_in_paper/figureA3_re_vs_fe_coefplot.tiff'),dpi = 350,width = 7.5,height = 8,units = 'in')
...
outdir.tables = "policypolitics/tables_figures/tables/tables_in_paper/" 
tableout <-htmlTable(comp_coef)
sink(paste0(outdir.tables,"tableA4_appendix_compare_credible_intervals.html"))
