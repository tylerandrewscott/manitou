...
tdt = expand.grid(sort(unique(as.character(admin_districts$FOREST_ID))),unique(congress_ids$congress))
tdt = data.table(tdt)
setnames(tdt,c('FOREST_ID',"congress"))
temp_dt = tdt
temp_dt$FISCAL_YEARS = lapply(temp_dt$congress,function(x) congress_ids$FISCAL_YEAR[congress_ids$congress==x])
...
simulate_blurred_response = function(x){
  round(ifelse(is.na(x),NA,ifelse(!is.na(as.numeric(x)),as.numeric(x),ifelse(x=='a',runif(length(x),1,19),
   ifelse(x=='b',runif(length(x),20,99),
...
cbp_props = data.table(left_join(cbp_props,congress_ids))
cbp_props = cbp_props[,lapply(.SD,mean,na.rm=T),by=.(FOREST_ID,congress)][,-'FISCAL_YEAR',with=F]

setkey(cbp_props,FOREST_ID,congress)
setkey(temp_dt,FOREST_ID,congress)

temp_dt = data.table(left_join(temp_dt,cbp_props))
...
timber_sales = data.table(left_join(timber_sales,congress_ids))

timber_sales = timber_sales[,lapply(.SD,sum,na.rm=T),by = .(FOREST_ID,congress)]
timber_sales = timber_sales[!is.na(congress),-'FISCAL_YEAR',with=F]
setkey(temp_dt,FOREST_ID,congress)
setkey(timber_sales,FOREST_ID,congress)

temp_dt =  data.table(left_join(temp_dt,timber_sales))
...
county_voting_over = data.table(left_join(county_voting_over,congress_ids))
forest_voting_weighted = county_voting_over[,lapply(.SD,weighted.mean,w=Prop_Overlap,na.rm=T), by=.(FOREST_ID,congress),.SDcols = 'percentD_H']
setkey(forest_voting_weighted,FOREST_ID,congress)
setkey(temp_dt,FOREST_ID,congress)
...
setnames(county_covs,'Year','FISCAL_YEAR')
county_covs$FISCAL_YEAR = as.numeric(county_covs$FISCAL_YEAR)
county_covs = data.table(left_join(county_covs,congress_ids))

forest_weighted_covs = county_covs[,lapply(.SD,weighted.mean,w=Prop_Overlap), by=.(FOREST_ID,congress),.SDcols = 
                                     c("Unemp_Rate","Prop_SNAP_recipients","Population")]

setkey(forest_weighted_covs,FOREST_ID,congress)
setkey(temp_dt,FOREST_ID,congress)
temp_dt = data.table(left_join(temp_dt,forest_weighted_covs))
...
setkey(house,FISCAL_YEAR,congress, Congressional_District_ID)
...
house_overs = data.table(left_join(house_overs,congress_ids))
setkey(house_overs,FISCAL_YEAR,congress,Congressional_District_ID)
house_dt = house[house_overs,]
house_dt$Prop_Overlap = round(house_dt$Prop_Overlap,2)
house_dt = house_dt[Prop_Overlap>0,]
...
forest_house_values = house_dt[,lapply(.SD,weighted.mean,w=Prop_Overlap,na.rm=T), by=.(FOREST_ID,congress),.SDcols = pol_cols]

setkey(forest_house_values,'FOREST_ID','congress')
setkey(temp_dt,'FOREST_ID','congress')
temp_dt = data.table(left_join(temp_dt,forest_house_values))
temp_dt = temp_dt[congress %in% 108:115,]

use = readRDS('input/usfs_internal_data/forestUseAverage_2005-2014.RDS')
...
wildfire_burn = left_join(wildfire_burn,congress_ids)

wildfire_index = sapply(seq_along(temp_dt$congress),function(x) which(wildfire_burn$FISCAL_YEAR<temp_dt$congress_start_year[x]&
                                                                        wildfire_burn$FISCAL_YEAR>(temp_dt$congress_start_year[x]-6)))
...
temp_dt$congress_start_year = sapply(temp_dt$FISCAL_YEAR,min)
...
limit_use_index = sapply(seq_along(temp_dt$congress),function(x) which(limit_use$FISCAL_YEAR<temp_dt$congress_start_year[x]))
...
wilderness_index = sapply(seq_along(temp_dt$congress),function(x) which(wilderness_area$DESIG_YEAR<temp_dt$congress_start_year[x]))
...
temp_dt = merge(temp_dt,esa_counts)
...
#   else{hab_poly[hab_poly_intersects[[temp_dt$index_in_forestshp[i]]],]%>%filter(FISCAL_YEAR<temp_dt$congress_start_year[i]) %>%
...
#   else{hab_line[hab_line_intersects[[temp_dt$index_in_forestshp[i]]],]%>%filter(FISCAL_YEAR<temp_dt$congress_start_year[i]) %>%
#       filter(!duplicated(sciname)) %>% nrow(.)}})
...
fwrite(temp_dt,'input/prepped/national_forest_congress_covariates.csv')
