...
setnames(temp_count,'period_type',period_type)
  ...
  u_fe_calendar_year_id = dcast(data.table(index = 1:(length(u) + length(y)),CALENDAR_YEAR = c(temp_dt$CALENDAR_YEAR,rep(NA,length(y)))), index ~ CALENDAR_YEAR,fun.aggregate = length)
  u_fe_calendar_year_id[,`NA`:=NULL]
  u_fe_calendar_year_id[,index:=NULL]
  u_fe_calendar_year_id[length(u) + 1:length(y),]<-NA
  y_fe_calendar_year_id = dcast(data.table(index = 1:(length(u) + length(y)),CALENDAR_YEAR = c(rep(NA,length(u)),temp_dt$CALENDAR_YEAR)), index ~ CALENDAR_YEAR,fun.aggregate = length)
  y_fe_calendar_year_id[,`NA`:=NULL]
  y_fe_calendar_year_id[,index:=NULL]
  y_fe_calendar_year_id[1:length(u),]<-NA
  names(y_fe_calendar_year_id) <- paste0('y_calendar_year_id_',names(y_fe_calendar_year_id))
  names(u_fe_calendar_year_id) <- paste0('u_calendar_year_id_',names(u_fe_calendar_year_id))
  ...
  idat[names(y_fe_calendar_year_id)]<-y_fe_calendar_year_id
  idat[names(u_fe_calendar_year_id)]<-u_fe_calendar_year_id
...
fe_form = update.formula(base_linear_form, as.formula(paste0("~ . - u_Ln_ACRES - y_Ln_Acres - -u_Ln_AVERAGE_YEARLY_VISITS - y_Ln_AVERAGE_YEARLY_VISITS- u_demPres - y_demPres - u_demCongress - y_demCongress - u_CountEorT_Species - y_CountEorT_Species - u_Perc_WUI_Housing - y_Perc_WUI_Housing + ",
                                                             paste(grep('forest_id_0102$|calendar_year_id_2005|state_id_AK|region_id_01',grep('_id_',names(idat),value = T),value = T,invert = T),collapse = '+'))))
...
saveRDS(mod_list,paste0('output/policypolitics/model_objects/fe_year_forest_models_',subtypes,'.RDS'))
